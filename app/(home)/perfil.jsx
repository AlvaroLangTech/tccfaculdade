...
export default function TelaPerfil() {
  ...
  // Busca dados do perfil no Firestore ao carregar a tela
  useEffect(() => {
    ...
    if (usuario) buscarPerfil();
  }, [usuario]);

  // Função para salvar as alterações (HU3)
  const salvarPerfil = async () => {
    if (!nome.trim()) return Alert.alert('Erro', 'O nome é obrigatório');

    setSalvando(true);
    try {
      // Atualiza no Auth (sessão) e no Firestore (banco)
      await updateProfile(auth.currentUser, { displayName: nome, photoURL: foto });
      await updateDoc(doc(db, 'usuarios', usuario.uid), { nome, fotoPerfil: foto });
  ...
  return (
    <ScrollView style={estilos.container} contentContainerStyle={estilos.conteudo}>
      ...
      <CardFormulario titulo="Editar Perfil">
        ...
        <BotaoPrimario titulo="Salvar Alterações" onPress={salvarPerfil} carregando={salvando} cor="#2563EB" />
      </CardFormulario>
